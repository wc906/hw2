...
int beta;//change datatype from float to integer
MPI_Request reqs[10];
MPI_Status stats[10];
...
if (rank == 1) {
  for (i=0; i<10; i++) {
    ...
    //datatype sent and received should be consistent, rank 0 send integer
    //rank 1 should also receive integer
    MPI_Wait(&reqs[i], &stats[i]);
    printf("Task %d received = %d\n",rank,beta);
    //should print integer too
    }
  }
