...
if (rank == 0) {
  ...
  while (1) {
    ...
    //don't quite understand yet. I guess mpi_send returned before mpi_recv finished
    //this may cause the job after mpi_recv accumulates in one send timing periodically.
    //change mpi_send to mpi_Ssend will synchronize send and receive and can solve the problem
    count++;
    if (count % 10 == 0) {
      end = MPI_Wtime();
